...
void TCPInit(void) {
  ...
  tcp_buf = (uint8_t*) malloc(BUFFER_SIZE);
  if (!tcp_buf) {
    DEBUG_PRINTLN("could not allocate buffer");
    return;
  }

  if (server_tcp) {
    DEBUG_PRINTLN("Stopping TCP server");
    server_tcp->stop();
    delete server_tcp;
    server_tcp = nullptr;
  ...
  if (tcp_port > 0) {
    DEBUG_PRINT("Starting TCP server on port ");
    DEBUG_PRINTLN(tcp_port);
...
void TCPLoop(void)
{
  ...
  do {
    tcp_busy = false;       // exit loop if no data was transferred

    if (buf_len > 0) {
      DEBUG_PRINT("from RFLink: ");
      DEBUG_PRINTLN(buf_len);
    ...
    for (uint32_t i = 0; i < ARRAY_SIZE(client_tcp); i++) {
      ...
      while (client && (buf_len < BUFFER_SIZE) && (client.available())) {
        rc = client.read();
        if (isAscii(rc)) { // ensure char is ascii, this is to stop bad chars being sent https://www.arduino.cc/en/Tutorial/CharacterAnalysis
          BUFFER_TCP[i][BUFFER_TCP_LEN[i]] = rc;
          DEBUG_PRINT(F("Data "));
          DEBUG_PRINT(rc)
          DEBUG_PRINT(F(" from TCP conn:"));
          DEBUG_PRINT(i);
          DEBUG_PRINT(F(" to put on index: "));
          DEBUG_PRINTLN(BUFFER_TCP_LEN[i]);
          BUFFER_TCP_LEN[i]++;
          if (BUFFER_TCP[i][BUFFER_TCP_LEN[i] - 1] == '\n') {
            TCP_DATA_READY[i] = true;
            BUFFER_TCP[i][BUFFER_TCP_LEN[i]] = '\0';
            DEBUG_PRINT(F("Detected end of line on TCP conn: "));
            DEBUG_PRINT(i);
            DEBUG_PRINT(F(" on index: "));
            DEBUG_PRINTLN(BUFFER_TCP_LEN[i]);
          }
        }
      }
      if (BUFFER_TCP_LEN[i] > 0) {
        DEBUG_PRINT("from TCP socket: ");
        DEBUG_PRINTLN(BUFFER_TCP_LEN[i]);
      }
    }
...
void loop() {
  ...
  TCPLoop();                                                      // Listen for TCP server

  uint32_t i;
  for (i = 0; i < ARRAY_SIZE(client_tcp); i++) {
    if (TCP_DATA_READY[i]) {
      DEBUG_PRINT(F("Data to send from TCP conn:"));
      DEBUG_PRINT(i);
      DEBUG_PRINTLN(F(" to Serial"));
      rflinkSerialTX.write(BUFFER_TCP[i], BUFFER_TCP_LEN[i]);            // Write command to RFLink serial
      rflinkSerialTX.print(F("\r\n"));
      TCP_DATA_READY[i] = false;
      BUFFER_TCP_LEN[i] = 0;
    }
  }
